...
from gammainc import mygammainc
...
def sample_finite_GBFRY(alpha, tau, mu=1., shape=(1, 5)):
    ...
    return (c * torch.exp(log_w) * pareto_mat * gamma_mat).detach()
...
def sample_finite_GGP(alpha, beta=1, mu=1., shape=(1, 5)):
    ...
    return (torch.exp(log_w) * gamma_mat).detach()
...
class GBFRYInitLearnableAlpha(nn.Module):
    def __init__(self, tau=2, mu=1):
        super().__init__()
        self.is_static = False
        self.alpha_logit = nn.Parameter(0.01*torch.randn(1))
        self.tau = tau
        self.mu = mu
    ...
    #    c = self.mu * (self.tau - 1) / (self.tau - alpha)
    #    return (-(1+self.tau)*x.log()+torch.log(torch.special.gammainc(
    #        torch.tensor(self.tau-alpha).to(x.device), x/c)))
    ...
    def log_pdf(self, x):
        ...
        #tl = (alpha * in_features * self.tau / (self.tau-alpha))**(1 / alpha)
        #g_in = torch.tensor(self.tau-alpha).to(x.device)
        #return (-(1+self.tau)*x.log()+torch.log(
        #    torch.special.gammainc(g_in, x_) -  torch.special.gammainc(g_in, x_*(tl+1))/(1+tl)**(self.tau-alpha)))
...
def lam_dist(p, name, tau=2, c=1, eta=1):
    # this function returns a sampler for the variances lambda
    if name == 'iid':
        return IIDInit(p)
...
def lam_sampler(p, name, tau=2, c=1, eta=1):
