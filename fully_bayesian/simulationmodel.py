...
class BayesLinear(nn.Linear):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        lam = self.get_variances().sqrt().float()
        active = torch.ones_like(lam).float()
        if not self.training:
            active = lam >= self.trunc_eps
        return F.linear(input.float(), (self.weight*lam*active).float(), self.bias.float())

    def get_variances(self):
        #print(self.transformed_variances.detach())
        return self.var_dist.map_to_domain(self.transformed_variances).cuda()
